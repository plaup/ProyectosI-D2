...
router.get('/:id', function(req, res, next) {
	Carreras.findOne( { _id: ObjectId.fromString(req.params.id) }, function( err, carrera ){
		if(err) res.send(500, err.message);
...
router.post('/:id', function(req, res, next) {
	var carrera = new Carreras({
		...
		campus: 				ObjectId.fromString(req.body.campus)
	});
...
router.put('/:id', function(req, res, next) {
	Carreras.findOne( { _id: ObjectId.fromString(req.params.id) }, function(err, carrera){
		...
		carrera.campus				= ObjectId.fromString(req.body.campus);
