...
router.get('/:id', function(req, res, next) {
	Horarios.findOne( { _id: ObjectId.fromString(req.params.id) }, function( err, horario ){
		if(err) res.send(500, err.message);
...
router.post('/:id', function(req, res, next) {
	var horario = new Horarios({
		...
		dia: 			req.body.dia,
		salon: 			ObjectId.fromString(req.body.salon),
...
router.put('/:id', function(req, res, next) {
	Horarios.findOne( { _id: ObjectId.fromString(req.params.id) }, function(err, horario){
		...
		horario.salon		= ObjectId.fromString(req.body.salon);
		horario.curso		= ObjectId.fromString(req.body.curso);
