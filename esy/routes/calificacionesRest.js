...
router.get('/:id', function(req, res, next) {
	Calificaciones.findOne( { _id: ObjectId.fromString(req.params.id) }, function( err, calif ){
		if(err) res.send(500, err.message);
...
router.post('/:id', function(req, res, next) {
	var calif = new Calificaciones({
		...
		alumno: 					ObjectId.fromString(req.body.alumno),
		curso: 						ObjectId.fromString(req.body.curso)
	});
...
router.put('/:id', function(req, res, next) {
	Calificaciones.findOne( { _id: ObjectId.fromString(req.params.id) }, function(err, calif){
		...
		calif.alumno 						= ObjectId.fromString(req.body.alumno);
		calif.curso 						= ObjectId.fromString(req.body.curso);
