...
router.get('/:id', function(req, res, next) {
	Prerrequisitos.findOne( { _id: ObjectId.fromString(req.params.id) }, function( err, prerreq ){
		if(err) res.send(500, err.message);
...
router.post('/:id', function(req, res, next) {
	var prerreq = new Prerrequisitos({
		...
		prerrequisito: 	ObjectId.fromString(req.body.prerrequisito)
	});
...
router.put('/:id', function(req, res, next) {
	Prerrequisitos.findOne( { _id: ObjectId.fromString(req.params.id) }, function(err, prerreq){
		prerreq.asignatura	= ObjectId.fromString(req.body.asignatura);
		prerreq.prerrequisito	= ObjectId.fromString(req.body.prerrequisito);
